...
// Carrossel adaptado ao design da REDEGEST (paleta madeira, tokens do site).
// Deslize por arrasto/toque, setas e indicadores. Ideal para muitas fotos.
...
function CarouselNavigation({ className, classNameButton, alwaysShow }: CarouselNavigationProps) {
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute top-1/2 left-0 flex w-full -translate-y-1/2 justify-between px-2",
        className
      )}
    >
      <button
        ...
        className={cn(
          "pointer-events-auto flex h-9 w-9 items-center justify-center rounded-full bg-card/90 text-ink shadow-sm ring-1 ring-line backdrop-blur transition-opacity duration-300 hover:bg-card",
...
function CarouselIndicator({ className, classNameButton }: CarouselIndicatorProps) {
  ...
  return (
    <div className={cn("absolute bottom-2 z-10 flex w-full items-center justify-center", className)}>
      <div className="flex space-x-1.5 rounded-full bg-ink/30 px-2 py-1 backdrop-blur">
...
function CarouselContent({ children, className, transition }: CarouselContentProps) {
  ...
  return (
    <motion.div
      ...
      transition={
        transition || { damping: 20, stiffness: 110, type: "spring", duration: 0.2 }
      }
