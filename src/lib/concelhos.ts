// Distritos de Portugal (continente + regiões autónomas) e respetivos concelhos.
// Usado nos selects de localização (Distrito → Concelho).

export const CONCELHOS_POR_DISTRITO: Record<string, string[]> = {
  Aveiro: [
  ...
  Braga: [
    ...
    "Fafe", "Guimarães", "Póvoa de Lanhoso", "Terras de Bouro", "Vieira do Minho",
  ...
  Portalegre: [
    ...
    "Elvas", "Fronteira", "Gavião", "Marvão", "Monforte", "Nisa", "Ponte de Sor",
    "Portalegre", "Sousel",
  ],
  Porto: [
  ...
  Santarém: [
    ...
    "Ourém", "Rio Maior", "Salvaterra de Magos", "Santarém", "Sardoal", "Tomar",
  ...
  "Açores": [
    "Angra do Heroísmo", "Calheta (São Jorge)", "Corvo", "Horta", "Lagoa (Açores)",
...
/** Lista ordenada dos distritos. */
export const DISTRITOS_PT = Object.keys(CONCELHOS_POR_DISTRITO);

/** Concelhos de um distrito (vazio se distrito desconhecido). */
export function concelhosDe(distrito: string): string[] {
  return CONCELHOS_POR_DISTRITO[distrito] ?? [];
}
